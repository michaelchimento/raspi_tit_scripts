...
filepath = "/home/pi/APAPORIS/VIDEOS/"
filepathphotos = "/home/pi/APAPORIS/PHOTOS/"
filenamePrefix = name
...
def make_video(): # 10 photos
	...
	camera.awb_mode = 'tungsten' 
        camera.ISO = (0)
        #time.sleep(0)
        timest = (name + '_{:%Y-%m-%d_%H%M%S}'.format(datetime.now()))
        filename = str(timest + '.h264')
        #filename=str('Aviary' + timest + '.jpg')
	...
	while (datetime.now()-start).seconds < 180:
        ...
        #print(count)
        #time.sleep(2400)
        move_command = str("mv " + filepath + filename + str(" APAPORIS/MOVED/"))
